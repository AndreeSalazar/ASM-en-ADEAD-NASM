...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Compile { input, output, backend } => {
            ...
            // PIPELINE INTELIGENTE: Análisis automático y selección óptima
            println!("   🔍 Analizando código ADead y seleccionando pipeline óptimo...");
            
            // Usar pipeline selector inteligente
            match adead_parser::pipeline_selector::process_adead_intelligent(&source) {
                Ok((pipeline, nasm_code)) => {
                    ...
                    if !nasm_code.is_empty() && (nasm_code.contains("section") || nasm_code.len() > 100) {
                        fs::write(&output_path, nasm_code)
                            .with_context(|| format!("Failed to write output file: {}", output_path))?;
                        println!("✅ Compilado ({}): {} -> {}", pipeline_name, input, output_path);
                        return Ok(());
                    } else {
                        println!("   ⚠️ Pipeline {} generó código incompleto, intentando fallback...", pipeline_name);
                    }
                }
                Err(e) => {
                    println!("   ⚠️ Pipeline inteligente falló: {}, intentando fallback...", e);
                }
            }
            
            // FALLBACK: Intentar pipelines individuales
            let input_path = Path::new(&input);
            let current_dir = input_path.parent();
            
            // PRIORIDAD 1: Zig directo para while loops (más confiable)
            let has_complex_structures = source.contains("while") || source.contains("if") || source.contains("loop");
            let has_while = source.contains("while");
            
            if has_while || has_complex_structures {
                println!("   ⚡ Fallback: Intentando Zig → NASM directo...");
                if let Some(nasm_code) = adead_parser::zig_nasm_generator::generate_nasm_direct(&source) {
                    if nasm_code.contains("loop") || nasm_code.contains("jmp") || nasm_code.contains("cmp") || nasm_code.len() > 500 {
                        fs::write(&output_path, nasm_code)
                            .with_context(|| format!("Failed to write output file: {}", output_path))?;
                        println!("✅ Compilado (Zig → NASM directo): {} -> {}", input, output_path);
                        return Ok(());
                    }
                }
