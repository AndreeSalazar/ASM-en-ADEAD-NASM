...
pub const StatementParser = struct {
    ...
    fn skipWhitespace(self: *StatementParser) void {
    ...
    fn parseWhile(self: *StatementParser) !?Statement {
        const old_pos = self.pos;
        ...
        var body_statements = std.ArrayList(Statement).init(self.allocator);
        errdefer body_statements.deinit();
        
        while (body_parser.parse()) |stmt| {
    ...
    fn parseIf(self: *StatementParser) !?Statement {
        const old_pos = self.pos;
        ...
        return Statement{
            .If = .{
                .condition = condition.?,
                .then_body = try body_statements.toOwnedSlice(),
                .else_body = null,
            },
        };
    }
    ...
    pub fn parse(self: *StatementParser) !?Statement {
        ...
        if (self.parseIf()) |stmt| {
            return stmt;
        } else |_| {}
        
        // Intentar parsear while
        if (self.parseWhile()) |stmt| {
            return stmt;
        } else |_| {}
