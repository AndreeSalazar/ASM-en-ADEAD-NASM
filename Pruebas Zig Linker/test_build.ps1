# Script para probar el comando build
$ErrorActionPreference = "Stop"

$workspace = "C:\Users\andre\OneDrive\Documentos\ASM-en-ADEAD-NASM"
Set-Location $workspace

Write-Host "`n=== PRUEBA: Build con Zig ===" -ForegroundColor Cyan
Write-Host ""
...
if (-not (Test-Path $compiler_path)) {
    ...
    Set-Location (Join-Path $workspace "CORE\rust")
    cargo build --release --bin adeadc
...
Write-Host "`n[1/2] Compilando y linkeando con Zig..." -ForegroundColor Yellow
...
Write-Host "`n[2/2] Ejecutando programa..." -ForegroundColor Yellow
...
Write-Host "`nCódigo de salida: $exec_exit" -ForegroundColor $(if ($exec_exit -eq 0) { "Green" } else { "Yellow" })
