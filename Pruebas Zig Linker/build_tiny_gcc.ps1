# ADead - Build TINY con GCC (PowerShell)
...
$exeFile = Join-Path $scriptDir "test_simple_tiny.exe"
$exeFileUpx = Join-Path $scriptDir "test_simple_tiny_upx.exe"

Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "  ADead - Build TINY (GCC Optimizado)" -ForegroundColor Cyan
Write-Host "========================================`n" -ForegroundColor Cyan
...
# Paso 2: Linkear con GCC
Write-Host "[2/4] Linkeando con GCC (flags optimizados)..." -ForegroundColor Yellow
& gcc -nostdlib -s `
    -Wl,--strip-all,--gc-sections,--file-alignment=16,--section-alignment=16,--no-seh `
    $objFile -lkernel32 -o $exeFile

if ($LASTEXITCODE -ne 0) {
    Write-Host "ERROR: Fallo al linkear con GCC" -ForegroundColor Red
    Write-Host "  Verifica que GCC (MinGW) esté instalado y en PATH" -ForegroundColor Yellow
    exit 1
}
